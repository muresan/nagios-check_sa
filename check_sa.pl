...
sub read_logs {
    ...
    my $interval = 60;
    my $count = $opts->{'minutes'} * 60 / $interval;
    
    my ($s_s, $s_m, $s_h, $s_d) = localtime(time() - ($interval * $opts->{'minutes'})); 
    my $start_time = sprintf("%02d:%02d:00", $s_h, $s_m);
    my @sa_log_files = ();

    my ($ts, $tm, $th, $td) = localtime(time());
    my ($ys, $ym, $yh, $yd) = localtime(time() - (24 * 60 * 60));
    my $yd_log_file = "$sa_log_dir/sa".sprintf("%02d", $yd);
    ...
    foreach my $log (@sa_log_files) {
        $sadf_output .= `$sadf -s $start_time -- -Adp $interval $count $log`;
    }
    ...
    $sadf_output .= `$sadf -s $start_time -- -Adp $interval $count`;
...
$opts{'sa_log_dir'} = $npo->get('sa-log-dir') || '/var/log/sa';
